...
Populate missing profile data for existing users
"""
...
def update_user_profile(username, profile_data):
    """Update user profile using the registration endpoint (it should update existing users)"""
    print(f"🔄 Updating profile for: {username}")
    
    try:
        # Try to update profile by calling register with existing username
        response = requests.post(f"{PRODUCTION_BASE_URL}register/", json=profile_data, timeout=10)
        
        if response.status_code == 201:
            print(f"✅ Profile updated successfully for {username}")
            return True
        elif response.status_code == 400:
            # User already exists, try to get current profile first
            print(f"⚠️  User {username} already exists, checking current profile...")
            
            # Check current profile
            profile_response = requests.get(f"{PRODUCTION_BASE_URL}get_user_profile/{username}/", timeout=10)
            if profile_response.status_code == 200:
                current_profile = profile_response.json()
                if current_profile.get('full_name') or current_profile.get('university'):
                    print(f"✅ Profile already has data for {username}")
                    return True
                else:
                    print(f"❌ Profile is empty for {username}, but registration failed")
                    return False
            else:
                print(f"❌ Could not check profile for {username}")
                return False
        else:
...
def main():
    ...
    print("🔧 PinIt User Profile Fixer")
    print("=" * 60)
    ...
    if successful_updates > 0:
        ...
        print(f"   This might be due to backend restrictions on updating existing users.")
