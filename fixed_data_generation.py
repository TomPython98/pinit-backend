#!/usr/bin/env python3
"""
Fixed test data generation for PinIt app
Based on thorough analysis of backend endpoints and their actual behavior
"""
...
def invite_to_event(event_id, username):
    """Invite a user to an event - using correct URL without /api/ prefix"""
    url = f"{BASE_URL}/invite_to_event/"  # Note: no /api/ prefix based on backend analysis
...
def submit_user_rating(reviewer, reviewee, rating, comment):
    """Submit a user rating - fixed to check for success in response body"""
...
def main():
    print("🚀 Creating comprehensive test data with FIXED endpoints...")
    ...
    # Create invitations to events - using FIXED endpoint
    ...
    # Create user ratings - using FIXED success detection
    ...
    print("\n🎉 FIXED test data generation finished!")
    print(f"\n📊 Summary:")
